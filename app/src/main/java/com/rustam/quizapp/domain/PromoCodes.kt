...
/**
 * A promo code and the [reward] it grants.
 * [infinite] = true means the code can be redeemed repeatedly (no AlreadyRedeemed gate).
 * Codes are matched case-insensitively.
 */
data class PromoCode(val code: String, val reward: PromoReward, val infinite: Boolean = false)

/**
 * Catalogue of redeemable promo codes.  One-time codes are recorded in the `redeemed_promo` table
 * so they can only be claimed once per install.  [infinite] codes bypass that check and may be
 * redeemed as many times as the player wants.
 *
 * To add a campaign code, append a [PromoCode] here; no other change is needed.
 */
object PromoCodes {
    val all: List<PromoCode> = listOf(
        // ── Infinite developer / test codes ───────────────────────────────────
        PromoCode("AAAAAA", PromoReward(coins = 10_000), infinite = true),
        PromoCode("BBBBBB", PromoReward(xp    = 10_000), infinite = true),
        PromoCode("CCCCCC", PromoReward(gems  = 10_000), infinite = true),
        // ── Combos ────────────────────────────────────────────────────────────
